...
# Load data
#gwas_raw <- read_tsv(
#    "/home/rrunyan1/Rose/results/nemascan/run1/elegans/LOCO/Mapping/Raw/LENGTH_ESTIMATE_lmm-exact.loco.mlma",
#  col_types = cols()
#) 


qtl_peaks <- read_tsv(
  "/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Mapping/Processed/QTL_peaks_inbred.tsv"
)
# isolate chr I QTL 
qtl_chrI <- qtl_peaks %>%
  filter(CHROM == "I") %>%
...
region_start <- qtl_chrI$startPOS / 1e6
region_end   <- qtl_chrI$endPOS  / 1e6
...
region_start <- qtl_chrIV$startPOS / 1e6
region_end   <- qtl_chrIV$endPOS  / 1e6

# Load aggregate mapping
d.all <- fread("/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Mapping/Processed/processed_LENGTH_ESTIMATE_AGGREGATE_mapping_inbred.tsv") %>%
...
# Add gene labels
genes <- data.frame(
  CHROM = c("I", "I", "IV", "III", "II"),
  start = c(2980383,15329043, 11520337, 7887655, 4619829),
  end   = c(2984432, 15332022, 11522404, 7889226, 4623231),
  gene  = c("mrt-1.1", "mrt-1.2", "sun-1", "pot-1", "CBG11106")
)
...
ggplot2::ggsave(
  filename = "/home/rrunyan1/Rose/plots/briggsae/briggsae_run1_man_plot.png",
  plot = manhattan_plot,
  width = 5,      # in inches
  height = 3,     # in inches
...
# P x G chrI
#────────────────────────────────────────────────
peak_snp <- qtl_chrI$marker   
#trait_data <- read_csv("/vast/eande106/projects/Maya/alcohol_sensitivity/data/20240609_rawdata_Fig1_2.csv") 
trait_data <- read_tsv("~/Rose/results/nemascan/run1/briggsae/Phenotypes/pr_LENGTH_ESTIMATE.tsv")
allele_data <- read.delim('/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Mapping/Processed/processed_LENGTH_ESTIMATE_AGGREGATE_mapping_inbred.tsv')
...
pxg_plot
ggplot2::ggsave(
  filename = "/home/rrunyan1/Rose/plots/briggsae/pxg_plot_I_.png",
  plot = pxg_plot,
  width = 5,      # in inches
  height = 5,     # in inches
  dpi = 300       # high resolution
...
# P x G chrIV
#────────────────────────────────────────────────
peak_snp_IV <- qtl_chrIV$marker   
#trait_data <- read_csv("/vast/eande106/projects/Maya/alcohol_sensitivity/data/20240609_rawdata_Fig1_2.csv") 
trait_data <- read_tsv("~/Rose/results/nemascan/run1/briggsae/Phenotypes/pr_LENGTH_ESTIMATE.tsv")
allele_data <- read.delim('/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Mapping/Processed/processed_LENGTH_ESTIMATE_AGGREGATE_mapping_inbred.tsv')
allele_peak_IV <- allele_data %>%
  filter(marker == peak_snp_IV)
# Merge and format allele data
combined_data_IV <- merge(trait_data, allele_peak_IV, by = "strain") %>%
  mutate(
    allele = factor(dplyr::recode(as.character(allele), "-1" = "REF", "1" = "ALT"), levels = c("REF", "ALT")),
    value = LENGTH_ESTIMATE
  )
...
pxg_plot
ggplot2::ggsave(
  filename = "/home/rrunyan1/Rose/plots/briggsae/pxg_plot_IV_.png",
  plot = pxg_plot,
  width = 5,      # in inches
  height = 5,     # in inches
  dpi = 300       # high resolution
...
annotations_I <- data.table::fread("/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Fine_Mappings/Data/LENGTH_ESTIMATE.I.13680791.15490148.LD_inbred.tsv")
annotations_IV <- data.table::fread("/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Fine_Mappings/Data/LENGTH_ESTIMATE.IV.13003613.17063404.LD_inbred.tsv")
...
finemap_I <- data.table::fread("/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Fine_Mappings/Data/LENGTH_ESTIMATE.I.13680791.15490148.finemap_inbred.inbred.fastGWA")
finemap_IV <- data.table::fread("/home/rrunyan1/Rose/results/nemascan/run1/briggsae/INBRED/Fine_Mappings/Data/LENGTH_ESTIMATE.IV.13003613.17063404.finemap_inbred.inbred.fastGWA")
...
# --- 3. LD with peak SNP ---
peak_variant_I <- annotations_I$PEAK_MARKER_I[1]  
peak_variant_I <- gsub("^5:", "", peak_variant_I)
finemap_I$VARIANT_LOG10p <- -log10(finemap_I$P)
# Define a small offset for the arrows (5% of max -log10(p))
max_logp <- max(finemap_I$VARIANT_LOG10p, na.rm = TRUE) * 0.05
...
annotations_unique_I <- unique(annotations_I[, .(POS, CONSEQUENCE)])
